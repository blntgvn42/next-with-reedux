  ...
  const filteredCountry =
    query === ''
      ...
      : countries.filter((country: Country) => {
        return country.name.toLowerCase().includes(query.toLowerCase()) || country.officialName.toLowerCase().includes(query.toLowerCase())
      })
            ...
            <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {filteredCountry.map((country) => (
                ...
                >
                  {({ active, selected }) => (
                    <>
                      <span className={clsx('block truncate', selected && 'font-semibold')}>{country.name}</span>
                      {selected && (
                        <span
                          className={clsx(
              ...
              <div className="w-full max-w-2xl xl:max-w-none xl:flex-auto xl:px-16 xl:py-24">
                <div className="flex flex-col justify-center">
                  ...
                  <div className='flex gap-2'>
                    {countryDetails.name.nativeName && Object.keys(countryDetails.name.nativeName).map((key) => (
                      <span key={key} className="inline-flex items-center rounded-md bg-yellow-50 px-2 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20">
                  ...
                  <blockquote className="text-xl font-semibold leading-8 text-white sm:text-2xl sm:leading-9">
                    <p>
                      ...
                      {countryDetails.borders && (
                        <>
                          There are {countryDetails.borders.length} countries bordering {countryDetails.name.common}. Those countries are {" "}
                          {countryDetails.borders.map((border: string) => countries.find((country) => country.cca3 === border)!.name + " ")}
                        </>
                      )
                      }
                    </p>
